...
let compute (type a) (module Equal : Equal with type t = a) (before : a list)
    (after : a list) =
  ...
  if max_d = 0 then []
    ...
    with Found (d_final, traces) ->
      ...
      for d = d_final downto 1 do
        ...
        if prev_k = k + 1 then ops := Line.Insert b.(prev_y) :: !ops
...
let hunks_of_lines ~context expected actual =
  ...
  let ops =
    compute (module Eq) (Array.to_list expected) (Array.to_list actual)
  in
...
let diff ?(context = 3) ?(expected_label = "expected")
  ...
  List.iter
    (fun h ->
      Buffer.add_string buf
        (Printf.sprintf "@@ -%d,%d +%d,%d @@\n" h.exp_start h.exp_len
           h.act_start h.act_len);
      let dels = ref [] in
      let adds = ref [] in
      List.iter
        (fun ((p, _) as line) ->
