...
(** [should_record_corrections ()] returns [true] if correction recording is
    ...
    ({!am_test_runner}) or when [INSIDE_DUNE] is set (enabling the
    [dune promote] workflow for executable-based expect tests). *)
...
(** [enter_group ?file ?tags name] starts a new test group. [file] is used for
    file-level scoping of test names. *)
