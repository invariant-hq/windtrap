...
(** The JUnit renderer: run results as a JUnit XML report.
    ...
    - {b Expected failures} (amendment B12). JUnit knows no xfail state, so an
      {e excused} failure — an [xfail] test that failed, listed in {!render}'s
      [excused] — becomes a [skipped] testcase whose message names the
      expectation ([expected failure: issue #42]); its real failures are not
      emitted, matching the run they did not fail. An [xfail] test that
      {e passed} arrives as an ordinary counted failure whose message names the
      reason and needs no mapping.
...
val render_full :
  ...
  unit ->
  string
(** [render_full ~excused ~suite ~results ~duration ()] is the complete XML
    document — [<?xml ?>] declaration, final newline included — for [results],
    in list order.
    ...
    entries, and excused failures count as [skipped] (see the preamble);
    [errors] is always [0] — windtrap classifies every failure kind as a JUnit
    failure. [duration] is the run's wall-clock seconds, emitted as the suite
    [time]; per-case times come from each result.

    [excused] lists the run's expected failures by joined path
    ({!Test_tree.path_to_string}) with their [xfail] annotations, exactly as for
    {!Render.finish}; pass [[]] for a run without [xfail] tests.
...
val render :
  ...
  unit ->
  string
(** [render ~suite ~results ~duration ()] is
    {!render_full}[ ~excused:[] ~suite ~results ~duration ()] — the report of a
    run with no expected failures. *)
